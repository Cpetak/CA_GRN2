...
def evolutionary_algorithm(pop_size, grn_size, num_cells, dev_steps, mut_rate, num_generations, mylambda, selection_prop, rules, mut_size, folder, seed_ints, season_len, job_array_id):
  ...
  for idx, seed in enumerate(seeds):
    targets.append(helper.rule2targets_wrapped_wstart(int(rules[0]), L=dev_steps+1, N=num_cells, start_pattern=seed))
  ...
  for gen in trange(num_generations):
    ...
    if fit_blast and season_counter < 3:
      ...
      fitnesses = helper.fitness_function_ca(p, rand_target)
    else:
      fitnesses = helper.fitness_function_ca(p, targets[curr])
    fitnesses=1-(fitnesses/worst) #0-1 scaling
    ...
    fitnesses_to_use = fitnesses #- pop_sum
    ...
    #Logging
    best_grn = pop[perm[0]]
    max_fit=fitnesses.max().item()
    ave_fit=fitnesses.mean().item()
